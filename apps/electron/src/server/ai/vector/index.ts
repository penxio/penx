...
export class PgLiteVector extends MastraVector<PGVectorFilter> {
  private db: PGlite
  ...
  async query({
    ...
    try {
      ...
      if (
        ...
        !queryVector.every((x) => typeof x === 'number' && Number.isFinite(x))
      ) {
    ...
    await mutex.runExclusive(async () => {
      ...
      if (indexConfig.type === 'hnsw') {
        ...
        if (indexConfig.ivf?.lists) {
          ...
          const size = (
            (await client.query(`SELECT COUNT(*) FROM ${tableName}`)) as {
              rows: { count: number }[]
            }
          ).rows[0].count
  ...
  private async installVectorExtension(client: PGlite) {
    ...
    if (!this.installVectorExtensionPromise) {
      this.installVectorExtensionPromise = (async () => {
        try {
          ...
          this.vectorExtensionInstalled = (
            extensionCheck.rows[0] as {
              exists: boolean
            }
          ).exists
  ...
  async describeIndex({
    ...
    try {
      ...
      return {
        dimension: (dimResult.rows[0] as { dimension: number }).dimension,
