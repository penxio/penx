...
const inputVariants = cva('', {
  variants: {
    ...
    variant: {
      ...
      panel:
        'h-9 border-none text-right shadow-none focus-visible:border-none focus-visible:ring-0 dark:bg-transparent',
      filled:
...
function Input({ className, size, variant, type, ...props }: InputProps) {
  return (
    <input
      ...
      data-slot="input"
      className={cn(
        'file:text-foreground placeholder:text-foreground/30 selection:bg-primary selection:text-primary-foreground dark:bg-input/30 border-input text-foreground flex h-9 w-full min-w-0 rounded-md border bg-transparent px-3 py-1 text-base outline-none transition-[color,box-shadow] file:inline-flex file:h-7 file:border-0 file:bg-transparent file:text-sm file:font-medium disabled:pointer-events-none disabled:cursor-not-allowed disabled:opacity-50 md:text-sm',
