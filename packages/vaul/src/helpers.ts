...
export function getSafePaddingTop(): string {
  if (isIOS()) return 'env(safe-area-inset-top)'
  const root = document.documentElement
  const val = getComputedStyle(root)
    .getPropertyValue('--safe-area-inset-top')
    .trim()
  const matches = val.match(/max\((\d+)px,\s*(\d+)px\)/)
  if (matches) {
    const v1 = parseInt(matches[1], 10)
    const v2 = parseInt(matches[2], 10)
    return `${Math.max(v1, v2)}px`
  } else {
    return '32px'
  }
}
