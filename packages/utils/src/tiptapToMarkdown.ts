...
export class TiptapToMarkdown {
  ...
  convertFromString(doc: string) {
    const jsonContent: TiptapDocument = JSON.parse(doc)
  ...
  convertNode(node: TiptapNode): string {
    switch (node.type) {
      ...
      case 'taskList':
        return (
          node.content?.map((item) => this.convertNode(item)).join('') || ''
        )
      ...
      case 'taskItem': {
        ...
        const lines = node.content
          ? node.content.map((child) => this.extractText(child)).join('')
          : ''
        return `- [${checked}] ${lines}\n`
      }

      case 'image': {
        const { src, alt, title, width, height } = node.attrs as any
  ...
  private convertList(listNode: TiptapNode, ordered: boolean = false): string {
    ...
    return listNode.content
      .map((item, index) => {
        ...
        const otherLines = lines.slice(1).map(
          (line) => (line ? '  ' + line : line), // Indent continuation lines
        )
...
export function tiptapToMarkdown(doc: TiptapDocument) {
  const tm = new TiptapToMarkdown()
  return tm.convert(doc)
}
